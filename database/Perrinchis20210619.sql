...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `personas` AS select `u`.`idUsuarios` AS `idUsuarios`,`d`.`Nombre` AS `Nombre`,`d`.`Apellidos` AS `Apellidos`,`td`.`NombTiDoc` AS `NombTiDoc`,`d`.`Cedula` AS `Cedula`,`g`.`Genero` AS `Genero`,`d`.`Direccion` AS `Direccion`,`d`.`Telefono` AS `Telefono`,`d`.`CorreElect` AS `CorreElect`,`d`.`FechaNace` AS `FechaNace`,`u`.`EstaUser` AS `EstaUser` from (((`usuarios` `u` join `datospersonales` `d` on((`d`.`idUsuarios` = `u`.`idUsuarios`))) join `generos` `g` on((`d`.`idGeneros` = `g`.`idGeneros`))) join `tipodocus` `td` on((`d`.`idTipoDocus` = `td`.`idTipoDocus`))) order by `u`.`idUsuarios` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-06-19 11:23:41
